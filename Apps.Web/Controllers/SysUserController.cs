...
namespace Apps.Web.Controllers
{
    public class SysUserController : BaseController
    {
        ...
        // GET: /SysUser/
        [Dependency]
        public ISysModuleRightBLL sysM_BLL { get; set; }
        [Dependency]
        ...
        #endregion  设置用户模块页面权限
        [SupportFilter(ActionName = "AllotRight")]
        public ActionResult GetRightByUser(string userId)
        {
            ViewBag.UserId = userId;
        ...
        [SupportFilter(ActionName = "AllotRight")]
        public JsonResult GetMuduleRightListByUser(string userId,string Id)
        {
            if (string.IsNullOrWhiteSpace(userId))
                return Json(0);
           List<P_Sys_GetModuleByUser_Result>  userList = sysM_BLL.GetModuleByUserId(userId,Id).ToList();
            ...
            var jsonData = (
                  ...
                  select new SysModuleRightModel ()
                {
                    ...
                    Rightflag = r.Rightflag == true ? true : false,
                    Enable = r.Enable,
                    ParentId = r.ParentId,
                    IsLast = r.IsLast,
        ...
        [SupportFilter(ActionName = "AllotRight")]
        [HttpPost]
        public JsonResult GetRightByUserAndModule(GridPager pager, string UserId, string ModuleId)
        {
            pager.rows = 100000;
            var right = sysM_BLL.GetRightByUserAndModule(UserId, ModuleId);
            var json = new
            {
                total = pager.totalRows,
                rows = (from r in right
                        select new P_Sys_GetRightByUserAndModule_Result()
                        {
                            Id= r.Id,
                            RightId = r.RightId,
                            KeyCode = r.KeyCode,
        ...
        #region 更新用户的module权限
        [SupportFilter(ActionName = "Save")]
        public int UpdateUserModuleRight(string UserId, string Modules)
        {
            return sysM_BLL.UpdateUserModule(UserId, Modules);
        }
        #endregion
        [SupportFilter(ActionName = "Save")]
        public int UpdateModuleOperateRight(SysModuleRightOperate Model)
        {
            return sysM_BLL.UpdataOperateRight(Model);
        }
        #region 更新用户选中module的操作权限
...
JsonRequestBehavior.AllowGet);
        ...
        public JsonResult Delete(string id)
        {
            if (!string.IsNullOrWhiteSpace(id))
            {
                ...
                if (m_BLL.Delete1(ref errors, id))
                {//删除用户要删除用户在模块、模块操作表里的所有记录。
