    ...
    public partial class SysUserBLL
    {
        ...
        public override bool Create(ref ValidationErrors errors, SysUserModel model)
        {
            try
            {
                ...
                //增加调用存储过程，新增userid,moduleid,rightflag
                if (m_Rep.Create(entity)&& (m_Rep.InsertUserModuleByUserId(model.Id)))
                {
                    return true;
                }
                else
        ...
        public  bool Delete1(ref ValidationErrors errors, string id)
        {
            try
            {
                if (m_Rep.DeleteUserMByUserid(id)&&(m_Rep.Delete(id) == 1))
                {
                    return true;
                }
                else
